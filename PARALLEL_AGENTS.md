...
**Line count:** Leaderboard.tsx went from 277 → 117 lines (–160 lines, –58%). Created 4 new sub-components totaling ~230 lines. Net increase is expected since extracted components include their own imports and type definitions.

**Recommendations for next run:**
- `getXpValue` and `getXpLabel` helpers are duplicated in TopThreeCard.tsx and LeaderboardRow.tsx. Could extract to a shared `leaderboard/utils.ts` if more components need them.
- Consider extracting the header section (gradient background + trophy icon + title) into a `LeaderboardHeader.tsx` to further reduce the parent to ~80 lines.
