...
## Run 50: Critical UI/UX Bug Fixes (8 Agents + Agent 0)

**Date**: 2026-02-13
**Agents**: 8 (A-H) + Agent 0
**Goal**: Fix ALL 8 user-reported bugs. Zero new features. Pure bug fixes.

**Bugs targeted:**
1. Onboarding blink/reload after each step
2. Dashboard quote overlaps stat cards
3. Header spacing — text touches top edge
4. Achievement badges clipped by overflow-hidden
5. Avatar not persisted from onboarding to profile
6. Avatar off-center in profile
7. Impossible quests (addressed in Run 51)
8. Punishment system transparency missing

---

### Run 50 Copy-Paste Prompts

**Agent A — Onboarding Blink Fix**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-a\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent A" section. You are Agent A.

YOUR TASK: Fix the onboarding blink/reload bug. After each step change, the page visually blinks with the same content before updating.

ROOT CAUSE: In mini-app/src/pages/Onboarding.tsx at line 198-208, the AnimatePresence wraps a motion.div with `key={store.currentStep}`. Every time currentStep changes, React fully unmounts and remounts the motion.div, causing a visible flash. The exit animation runs (opacity 0, x -20), then the NEW element mounts (opacity 0, x 20) and animates in — this creates a blink because the SAME content briefly disappears and reappears.

OWNED FILES (only you modify these):
- mini-app/src/pages/Onboarding.tsx

FIX:
1. Open the file and look at lines 198-208. You'll see:
   ```tsx
...
2. Replace with a smoother transition that doesn't unmount/remount:
   - Use `mode="popLayout"` instead of `mode="wait"` OR
   - Reduce the animation to just opacity (remove x movement) and shorten duration to 0.15s OR
   - Better approach: Use `layoutId` on the step container so Framer Motion animates between states without full remount. The simplest fix is to keep AnimatePresence but use `mode="sync"` and reduce the transition:
   ```tsx
   ...
   ```
   This makes the old step fade out while the new one fades in simultaneously — no gap.

3. Also check if `StepRenderer` or any child components cause additional re-renders. Look for any `useEffect` hooks that flash loading states.

FORBIDDEN: Do NOT modify any other files, bot/ files, or other components.

BUILD VERIFY: cd mini-app && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent A Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent B — Dashboard Layout Fix (Quote Overlap)**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-b\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent B" section. You are Agent B.

YOUR TASK: Fix the dashboard layout where the motivational quote overlaps with the stat cards below it.

ROOT CAUSE: In mini-app/src/pages/Dashboard.tsx:
- Line 87: Quote section uses `className="mx-4 -mt-4 relative z-10"` — the -mt-4 pulls the quote up
- Line 99: Stat grid uses `className="grid grid-cols-2 gap-3 px-4 -mt-8"` — the -mt-8 pulls stats up into the quote area
- These two negative margins combine to cause a visual overlap

OWNED FILES (only you modify these):
- mini-app/src/pages/Dashboard.tsx

FIX:
1. Line 87: Change `className="mx-4 -mt-4 relative z-10"` to `className="mx-4 mt-3 relative z-10"` (positive margin instead of negative)
2. Line 99: Change `className="grid grid-cols-2 gap-3 px-4 -mt-8"` to `className="grid grid-cols-2 gap-3 px-4 mt-4"` (positive margin instead of negative)
3. Check that the header gradient section above (around lines 60-86) doesn't rely on negative margin overlap for visual effect. If the gradient needs to extend behind the quote, use `pb-12` on the gradient container and `mt-[-2rem]` on the quote only (controlled overlap).
4. Test visually: the quote should be fully visible with clear separation from the stat cards below.

FORBIDDEN: Do NOT modify any other files, components, bot/ files.

BUILD VERIFY: cd mini-app && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent B Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent C — Header Spacing Fix (All Pages)**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-c\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent C" section. You are Agent C.

YOUR TASK: Fix header spacing across all pages. The page titles (QUESTS, ACHIEVEMENTS, etc.) nearly touch the top edge with no padding.

ROOT CAUSE: Most pages use a `safe-area-top` class on their header div, but the padding within that class may be insufficient. Additionally, Social.tsx has NO safe-area-top at all.

Current state of each page header:
- Quests.tsx line 48: `className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 rounded-b-3xl shadow-lg safe-area-top"`
- Achievements.tsx line 104: `className="bg-gradient-to-br from-amber-500 to-orange-600 p-6 rounded-b-3xl shadow-lg safe-area-top"`
- Leaderboard.tsx line 89: `className="bg-gradient-to-r from-yellow-500 to-orange-500 p-6 rounded-b-3xl shadow-lg safe-area-top"`
- Social.tsx line 222: `className="p-4"` — NO safe-area-top, NO gradient header
- Finance.tsx line 30: `className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 rounded-b-3xl shadow-lg safe-area-top"`
...
FIX:
1. First, check mini-app/src/index.css for the `.safe-area-top` class definition. It should add `padding-top: env(safe-area-inset-top, 0px)`. If the padding is too small, increase it.

2. For Social.tsx: Add a proper gradient header matching the other pages:
   - Wrap the h1 in a header container: `<div className="bg-gradient-to-r from-indigo-600 to-blue-600 p-6 rounded-b-3xl shadow-lg safe-area-top">`
   - Move the h1 and subtitle inside this container with white text

3. For ALL pages with safe-area-top (Quests, Achievements, Leaderboard, Finance):
   - Increase padding: change `p-6` to `pt-8 pb-6 px-6` (or add `pt-2` extra) to give more top space
   - This ensures the title text has breathing room from the top edge

4. Verify each page header looks consistent: gradient background, rounded bottom, white text, proper top padding.

FORBIDDEN: Do NOT modify Dashboard.tsx (Agent B owns it), Onboarding.tsx, Profile.tsx, Settings.tsx, or any bot/ files.

BUILD VERIFY: cd mini-app && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent C Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent D — Achievement Badge Clipping Fix**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-d\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent D" section. You are Agent D.

YOUR TASK: Fix the achievement badges ("NEW" and green checkmark / lock icon) that are partially clipped on achievement cards.

ROOT CAUSE: In mini-app/src/components/achievements/AchievementCard.tsx:
- Line 74: The card container has `overflow-hidden` in its className
- Lines 80-84: "NEW" badge uses `className="absolute -top-2 -left-2 ..."` — positioned OUTSIDE the card
- Lines 86-88: Green checkmark uses `className="absolute -top-1.5 -right-1.5 ..."` — also OUTSIDE
- Lines 90-92: Lock icon uses `className="absolute -top-1.5 -right-1.5 ..."` — also OUTSIDE
- The overflow-hidden clips these badges since they extend beyond the card boundaries
...
Option 1 (Recommended): Remove overflow-hidden from the card, keep badge positioning
- Line 74: Remove `overflow-hidden` from the className string
- Check if overflow-hidden was needed for any inner content. If the card has images or content that should be clipped, add overflow-hidden to just that inner element, not the whole card.

Option 2: Keep overflow-hidden, move badges inside
- Change badge positions from negative to positive:
  - "NEW": `absolute top-2 left-2` instead of `-top-2 -left-2`
  - Checkmark/Lock: `absolute top-2 right-2` instead of `-top-1.5 -right-1.5`
- This keeps badges fully inside the card but changes the visual design

Go with Option 1 unless there's a specific reason overflow-hidden is needed.

FORBIDDEN: Do NOT modify any other files, pages, bot/ files.

BUILD VERIFY: cd mini-app && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent D Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent E — Avatar Off-Center Fix in Profile**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-e\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent E" section. You are Agent E.

YOUR TASK: Fix the avatar display being off-center in the Profile page header.

ROOT CAUSE: In mini-app/src/components/profile/ProfileHeader.tsx:
- Line 41: `className="inline-block relative mb-4"` — inline-block doesn't guarantee centering
- Line 42-44: Avatar circle is 24x6 (w-24 h-24)
- There's a level badge at `absolute -bottom-2 -right-2` which shifts the visual weight to the right
- The parent div uses `text-center` but inline-block + absolute children can cause misalignment

OWNED FILES (only you modify these):
- mini-app/src/components/profile/ProfileHeader.tsx

FIX:
1. Change the avatar wrapper from `inline-block` to `flex items-center justify-center`:
   - Line 41: Change `className="inline-block relative mb-4"` to `className="flex items-center justify-center mb-4"`
   - Add an inner wrapper for the avatar circle + badge: `<div className="relative">`
...
3. The level badge stays in its absolute position relative to the avatar circle. The flex centering ensures the circle itself (not circle + badge) is the centered element.

FORBIDDEN: Do NOT modify ProfileEditModal.tsx, pages/Profile.tsx, or any bot/ files.

BUILD VERIFY: cd mini-app && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent E Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent F — Avatar Persistence Backend Fix**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-f\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent F" section. You are Agent F.

YOUR TASK: Fix the backend so that the avatar selected during onboarding is saved to the users table.

ROOT CAUSE: In bot/src/api/routes/onboarding.ts (the POST /:telegramId/complete endpoint, lines 90-227):
- The endpoint saves modes (lines 97-128), mode_configs (131-146), punishment_settings (149-167), XP (169-185), onboarding state (187-194), and daily quests (196-223)
- But it NEVER writes `avatar_id` to the `users` table
- The frontend AvatarSelect component stores the selection as a string like 'gym_warrior' in `quiz_data.gender`
- The users table has an `avatar_id INTEGER` column that defaults to NULL

OWNED FILES (only you modify these):
- bot/src/api/routes/onboarding.ts

FIX:
1. Add an avatar mapping and UPDATE query. After the XP award block (around line 185), before the onboarding state update (line 187), add:
...
2. Also save the nickname if present. The frontend sends `quiz_data.nickname` but it's never written:
```typescript
...
IMPORTANT: This runs INSIDE a transaction (line 96: `await withTransaction(async (client) => {`), so these queries join the existing transaction. Use `client.query()` not `query()`.

FORBIDDEN: Do NOT modify any mini-app files, other routes, or other bot files.

BUILD VERIFY: cd bot && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent F Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent G — Punishment Transparency in Onboarding**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-g\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent G" section. You are Agent G.

YOUR TASK: Add transparency text to the punishment/accountability section of onboarding explaining that XP depreciation happens even without the punishment system enabled.

ROOT CAUSE: Two components have unclear text:
1. mini-app/src/components/onboarding/PunishmentConfig.tsx lines 69-72:
   - Shows: "Want extra motivation? Pick a real consequence for skipping tasks."
   - Missing: No mention that XP already depreciates for missed quests even without this enabled

2. mini-app/src/components/onboarding/punishment/ConsentToggle.tsx lines 11-12:
   - Shows: "Enable accountability" / "Choose a real punishment you'll actually do"
   - Missing: What happens if you DON'T enable it

OWNED FILES (only you modify these):
- mini-app/src/components/onboarding/PunishmentConfig.tsx
- mini-app/src/components/onboarding/punishment/ConsentToggle.tsx

FIX:

1. In PunishmentConfig.tsx after line 72, add an info box:
```tsx
...
2. In ConsentToggle.tsx after line 12, add a subtitle:
```tsx
...
3. Make sure the added text is clear, concise, and doesn't scare users away. The tone should be informative, not threatening.

FORBIDDEN: Do NOT modify any bot/ files, other onboarding components, or pages.

BUILD VERIFY: cd mini-app && npm run build must pass.

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent G Retrospective", replacing the placeholder text. Then commit all changes.
```

**Agent H — Regression Tests for Bug Fixes**
```
Read c:\Users\Asus\Desktop\Wibecode-agent-h\PARALLEL_AGENTS.md — find "Run 50" and locate the "Agent H" section. You are Agent H.

YOUR TASK: Write regression tests that verify the bug fixes from this run. You are a TEST-ONLY agent — do NOT modify source files.

OWNED FILES (only you create these):
- mini-app/src/__tests__/regression/run50-bugs.test.tsx (NEW)

WHAT TO TEST:

1. **Onboarding no-blink**: Test that Onboarding.tsx renders the AnimatePresence with mode="sync" (or whatever Agent A used). Verify the motion.div transition is opacity-only (no x movement).

2. **Dashboard no-overlap**: Test that Dashboard.tsx stat grid does NOT use -mt-8. Render the Dashboard component and check the stat grid container's className.

3. **Header spacing**: Test that Social.tsx has safe-area-top class on its header container.

4. **Achievement badges visible**: Test that AchievementCard does NOT have overflow-hidden on its outer card container. Render a "new" achievement and verify the "NEW" badge element exists in the DOM (not clipped).

5. **Avatar centering**: Test that ProfileHeader renders the avatar wrapper with flex + justify-center classes.

6. **Punishment transparency**: Test that PunishmentConfig renders the info box about XP depreciation.

PATTERN: Read existing test files for mocking patterns:
- mini-app/src/__tests__/components/settings/DangerZone.test.tsx for component test pattern
- Mock framer-motion: vi.mock('framer-motion', ...)
- Mock lucide-react icons as needed
- Use render() + screen queries from @testing-library/react

Create a single test file with 6 describe blocks (one per bug fix). Each should have 1-3 focused tests.

Target: ~12-15 tests.

FORBIDDEN: Do NOT modify any source files. Only create test files.

BUILD VERIFY: cd mini-app && npx vitest --run src/__tests__/regression/run50-bugs.test.tsx

After completing work, write your retrospective in PARALLEL_AGENTS.md under "Run 50 Retrospectives" → "Agent H Retrospective", replacing the placeholder text. Then commit all changes.
```
...
**Branch:** `feature/r50-onboarding-blink`
**Worktree:** `../Wibecode-agent-a`
...
**FORBIDDEN:**
- All bot/ files, all other mini-app pages and components
- Dashboard.tsx, Profile.tsx, Settings.tsx

---

### Agent B — Dashboard Layout Fix

**Branch:** `feature/r50-dashboard-layout`
**Worktree:** `../Wibecode-agent-b`
...
**FORBIDDEN:**
- All bot/ files, all other pages and components
- Onboarding.tsx, Profile.tsx

---

### Agent C — Header Spacing Fix

**Branch:** `feature/r50-header-spacing`
**Worktree:** `../Wibecode-agent-c`
...
**FORBIDDEN:**
- All bot/ files, Dashboard.tsx (Agent B), Onboarding.tsx (Agent A)
- All components (only pages)

---

### Agent D — Achievement Badge Fix

**Branch:** `feature/r50-achievement-badges`
**Worktree:** `../Wibecode-agent-d`
...
**FORBIDDEN:**
- All bot/ files, all pages, all other components

---

### Agent E — Avatar Centering Fix

**Branch:** `feature/r50-avatar-centering`
**Worktree:** `../Wibecode-agent-e`
...
**FORBIDDEN:**
- All bot/ files, ProfileEditModal.tsx, pages/Profile.tsx

---

### Agent F — Avatar Persistence Backend

**Branch:** `feature/r50-avatar-backend`
**Worktree:** `../Wibecode-agent-f`
...
**FORBIDDEN:**
- All mini-app files, all other bot routes

---

### Agent G — Punishment Transparency

**Branch:** `feature/r50-punishment-info`
**Worktree:** `../Wibecode-agent-g`
...
### Agent H — Regression Tests

**Branch:** `feature/r50-regression-tests`
**Worktree:** `../Wibecode-agent-h`
...
### Run 50 File Ownership Matrix

| File / Directory | A | B | C | D | E | F | G | H |
|---|---|---|---|---|---|---|---|---|
| `mini-app/src/pages/Onboarding.tsx` | **OWNED** | - | - | - | - | - | - | - |
| `mini-app/src/pages/Dashboard.tsx` | - | **OWNED** | - | - | - | - | - | - |
| `mini-app/src/pages/Quests.tsx` | - | - | **OWNED** | - | - | - | - | - |
| `mini-app/src/pages/Leaderboard.tsx` | - | - | **OWNED** | - | - | - | - | - |
| `mini-app/src/pages/Social.tsx` | - | - | **OWNED** | - | - | - | - | - |
| `mini-app/src/pages/Finance.tsx` | - | - | **OWNED** | - | - | - | - | - |
| `mini-app/src/pages/Achievements.tsx` | - | - | **OWNED** | - | - | - | - | - |
| `AchievementCard.tsx` | - | - | - | **OWNED** | - | - | - | - |
| `ProfileHeader.tsx` | - | - | - | - | **OWNED** | - | - | - |
| `bot/src/api/routes/onboarding.ts` | - | - | - | - | - | **OWNED** | - | - |
| `PunishmentConfig.tsx` | - | - | - | - | - | - | **OWNED** | - |
| `ConsentToggle.tsx` | - | - | - | - | - | - | **OWNED** | - |
| `__tests__/regression/run50-bugs.test.tsx` | - | - | - | - | - | - | - | **NEW** |
| `PARALLEL_AGENTS.md` | retro | retro | retro | retro | retro | retro | retro | retro |

### Run 50 Merge Order

1. Agent F (backend — avatar persistence) — backend first
2. Agent A (onboarding blink) — frontend page
3. Agent B (dashboard layout) — frontend page
4. Agent C (header spacing) — multiple pages, no overlap
5. Agent D (achievement badges) — component
6. Agent E (avatar centering) — component
7. Agent G (punishment transparency) — component
8. Agent H (regression tests) — test only, merge LAST
