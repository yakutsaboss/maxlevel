...
describe('achievementNotifier', () => {
  ...
  it('should send notifications for new achievements', async () => {
    ...
    const unlocks = [
      { user_id: 1, telegram_id: 111, achievement_id: 10, name: 'First Quest', badge_icon: '🏅', xp_bonus: 25 },
      { user_id: 2, telegram_id: 222, achievement_id: 20, name: 'Streak Master', badge_icon: '🔥', xp_bonus: 50 },
    ];
    mockQuery
    ...
    expect(mockBot.api.sendMessage).toHaveBeenCalledWith(111, expect.stringContaining('First Quest'));
    expect(mockBot.api.sendMessage).toHaveBeenCalledWith(222, expect.stringContaining('Streak Master'));
  ...
  it('should handle Telegram API errors gracefully and count failures', async () => {
    ...
    const unlocks = [
      { user_id: 1, telegram_id: 111, achievement_id: 10, name: 'Test', badge_icon: '🏅', xp_bonus: 10 },
    ];
